...
for p = 1:Kout
    ...
    for i = 1:Kin
       ...
       for j = 1:N_pca
           % Initialization
            ...
            trainSet_in = [trainSet_in, train_in_pca(:,j)];
            testSet_in = [testSet_in, test_in_pca(:,j)];

            % Rank of features for inner loop, on training set
            [orderedIndin, orderedPowerin] = rankfeat(trainSet_in,trainLabels_in,'fisher');
    ...
    for j = 1:bestModel_PCA(p)
       trainSet_out_pca  = [trainSet_out_pca, train_out_pca(:,j)];
       testSet_out_pca  = [testSet_out_pca, test_out_pca(:,j)];
    end
